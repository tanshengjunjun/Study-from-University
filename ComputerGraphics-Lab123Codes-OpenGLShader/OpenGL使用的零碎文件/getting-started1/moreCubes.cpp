...
int main(int argc, char** argv)
{
	...
	// Section3 准备纹理对象
	GLint textureId = TextureHelper::load2DTexture("cat_ghost.png");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// 清除颜色缓冲区 重置为指定颜色
		glClearColor(0.18f, 0.04f, 0.0f, 1.0f);
		...
		glm::mat4 view = glm::lookAt(eyePos,
			glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));/*glm::vec3 getEyePosCircle();
	glm::vec3 getEyePosSphere();可放到上面代替eyePos后面的参数*/
		glm::mat4 model;
		// 绘制立方体
