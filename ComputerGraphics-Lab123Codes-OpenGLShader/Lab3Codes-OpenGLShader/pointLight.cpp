...
int main(int argc, char** argv)
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUniform3f(lightAmbientLoc, 0.5f, 0.5f, 0.5f);//环境光
		glUniform3f(lightDiffuseLoc, 0.7f, 0.7f, 0.7f);//漫反射
		glUniform3f(lightSpecularLoc, 1.0f, 1.0f, 1.0f);//镜面反射
		...
		glUniformMatrix4fv(glGetUniformLocation(shader.programId, "view"),
			1, GL_FALSE, glm::value_ptr(view));
		//下面部分是立方体OBJLOADER.cpp的
		glm::mat4 model2;
		model = glm::scale(model2, glm::vec3(0.8f, 0.8f, 0.8f));
		glUniformMatrix4fv(glGetUniformLocation(shader.programId, "model2"),
